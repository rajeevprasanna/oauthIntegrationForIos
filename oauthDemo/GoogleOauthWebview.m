...
#define accessTokenEndpoint         @"https://accounts.google.com/o/oauth2/token" @"https://accounts.google.com/o/oauth2/token"

@interface GoogleOauthWebview()
...
@implementation GoogleOauthWebview
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        //Set the access token and refresh token file paths
        ...
        //Make any other required initialization
...
-(void) authorizeUserWithClientID:(NSString *)client_ID andClientSecret:(NSString *)client_Secret andParentView:(UIView *)parent_View andScopes:(NSArray *)scopes{
    ...
    //Check if the access token file info exists or not
...
-(void) showWebviewForUserLogin{
    //Create a string to concatenate all scopes existing in the _scope array.
    NSString *scope = @"";
...
-(void)webViewDidFinishLoad:(UIWebView *)webView
{
    ...
    if([webviewTitle rangeOfString:@"Success code"].location != NSNotFound){
        ...
        //Show a "please wait ..." message to the web view
...
-(NSString *)urlEncodeString:(NSString *)stringToURLEncode{
    // URL encode the parameter and return it
